...
float3 HeightToNormal(Texture2D _tex, SamplerState _sampler, float2 _uv, float _intensity)
{
	float3 bumpSamples;
	...
	float dHdU = bumpSamples.z - bumpSamples.x;//bump U offset
	float dHdV = bumpSamples.y - bumpSamples.x;//bump V offset
